...
PROPÓSITO:
-----------
Este script analiza archivos CSV de experimentos de vibración forzada-amortiguada
para visualizar y validar la detección de eventos críticos en las señales de:
  - Canal A (LVDT): Señal cuadrada que indica la posición angular de la viga
  - Canal B (Proximidad): Señal sinusoidal que indica el paso de las masas excéntricas

FUNCIONAMIENTO:
---------------
1. Lee un archivo CSV específico con datos de oscilación del sistema
2. Detecta automáticamente:
   - Flancos ascendentes en la señal cuadrada del LVDT (transición bajo→alto)
   - Cruces por cero ascendentes en la señal sinusoidal del sensor de proximidad
3. Calcula el desfase φ entre ambas señales usando la diferencia temporal Δt
4. Compara tres opciones de cálculo para verificar cuál es la correcta:
   - Opción 1: φ = -(Δt/T)×2π  (fórmula actual del script principal)
   - Opción 2: φ = -(Δt invertido/T)×2π  (invirtiendo t_cruce - t_flanco)
   - Opción 3: φ = +(Δt/T)×2π  (sin signo negativo)
5. Genera tres gráficas superpuestas:
   - Canal A mostrando detección de flancos ascendentes
   - Canal B mostrando detección de cruces por cero
   - Comparación normalizada de ambas señales con medición visual de Δt

APLICACIÓN:
-----------
Este script es una herramienta de diagnóstico para:
  - Verificar que la detección de eventos funciona correctamente
  - Validar visualmente el desfase entre señales LVDT y proximidad
  - Depurar discrepancias entre valores experimentales y teóricos de φ
  - Confirmar qué fórmula de cálculo (con/sin -π/2) es la apropiada

USO:
----
1. Modificar la variable 'archivo' con la ruta del CSV a analizar
2. Ajustar 'frecuencia' al valor experimental correspondiente
3. Ejecutar el script: python visualizar_senales.py
4. Revisar la salida en consola y la imagen generada 'visualizacion_senales_phi.png'

NOTA IMPORTANTE:
----------------
Los valores de φ calculados aquí son para diagnóstico. Los valores finales
del informe se obtienen con el script 'calcular_phi.py' que procesa todos
los archivos CSV de cada configuración y calcula promedios.
"""
