"""
Cálculo del ángulo de desfase φ para experimentos de vibración forzada-amortiguada

PROPÓSITO:
-----------
Este script procesa TODOS los archivos CSV de experimentos de vibración forzada-amortiguada
para calcular el ángulo de desfase φ entre la señal de excitación (proximidad) y la
respuesta del sistema (LVDT). Los resultados se usan en el análisis del informe de laboratorio.
...
- El término -π/2 ajusta la referencia de fase según la convención experimental

FUNCIONAMIENTO:
---------------
1. Recorre todas las carpetas de configuraciones experimentales:
   - Forzada-amortiguada (cerrada): 4.2, 4.3, 4.45 Hz
   - Forzada-amortiguada-abierta: 4.2, 4.3, 4.45 Hz
2. Para cada archivo CSV:
   - Lee las señales del LVDT (Canal A - cuadrada) y proximidad (Canal B - sinusoidal)
   - Detecta el flanco ascendente del LVDT (transición bajo→alto)
   - Detecta el cruce por cero ascendente del sensor de proximidad
   - Calcula Δt = t_flanco_LVDT - t_cruce_proximidad
   - Aplica la fórmula para obtener φ en radianes y grados
3. Calcula promedios y desviaciones estándar por configuración/frecuencia
4. Exporta resultados en tabla CSV y muestra resumen en consola

ARCHIVOS DE ENTRADA:
--------------------
- CSVs en carpetas: G12/Forzada-amortiguada4.Xhz/ y G12/Forzada-amortiguada-abierta4.Xhz/
- Formato CSV: separador ';', decimal ',', skiprows=2
- Columnas: Tiempo(ms), Canal A(V), Canal B(mV)

ARCHIVOS DE SALIDA:
-------------------
- resultados_phi.csv: Tabla detallada con todos los valores calculados
- Salida en consola: Resumen por archivo y promedios por configuración

USO:
----
1. Asegurar que la ruta 'base_path' apunta correctamente a la carpeta G12
2. Verificar que existan las carpetas con datos experimentales
3. Ejecutar: python calcular_phi.py
4. Revisar resultados en consola y archivo resultados_phi.csv

NOTA:
-----
Los valores promedio de φ calculados por este script son los que aparecen
en las tablas de resultados del informe de laboratorio.
"""
